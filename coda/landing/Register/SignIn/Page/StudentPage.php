...
if (isset($_POST['query'])) {
  ...
  $search_faculty_query = "SELECT * FROM faculties WHERE names LIKE '$search_query'";
  $search_faculty_result = mysqli_query($conn, $search_faculty_query);

  if ($search_faculty_result && mysqli_num_rows($search_faculty_result) > 0) {
    // Display search results
    while ($row = mysqli_fetch_assoc($search_faculty_result)) {
      echo '<a class="search-result" href="portfolio.php?professor_id=' . $row['faculty_id'] . '">' . $row['names'] . '</a><br>';
    }
  } else {
    echo '<div style="color: white; text-align: center; padding-top: 15px; padding-bottom: 15px;">No results found</div>';
  }
...
<div class="search-container">
    <input type="text" id="searchInput" placeholder="Search a names...">
  </div>
